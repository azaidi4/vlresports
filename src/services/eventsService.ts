...
/**
 * Retrieves and parses event data based on the provided status, region, and page number from a specified URL using cheerio and fetch modules.
 ...
 */
export async function getEvents(status, region, page) {
  ...
  const $ = await fromURL(`${process.env.VLR_URL}/events/${region}?page=${page}`);
  const events = [];
