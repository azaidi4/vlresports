 ...
 */
export async function getPlayers(pagination, filters) {
  const startIndex =
  ...
  $('table')
    ...
    .map((i, el) => {
      ...
      const url = process.env.VLR_URL + $(el).find('td').find('a').attr('href');
      const country = $(el)
        .find('td')
        .find('.flag')
        .attr('class')
...
export async function getPlayerById(id) {
  ...
  const $ = await fromURL(`${process.env.VLR_URL}/player/${id}`);
  const $matches = await fromURL(
    `${process.env.VLR_URL}/player/matches/${id}/?group=completed`
  );
  ...
  const player = {
    id,
    url: `${process.env.VLR_URL}/player/${id}`,
    img,
    user,
    name,
    country,
  ...
  $matches('.m-item-result')
    ...
    .map((i, el) => {
      ...
      const match_url = process.env.VLR_URL + $(el).attr('href');
      const event_logo =
      ...
      const team1 = {
        ...
        logo: $(el)
          ...
          .includes('/img/vlr')
          ? process.env.VLR_URL +
            $(el).find('.m-item-logo img').first().attr('src')
      ...
      const team2 = {
        ...
        logo: $(el)
          ...
          .includes('/img/vlr')
          ? process.env.VLR_URL +
            $(el).find('.m-item-logo img').last().attr('src')
  ...
  $('.wf-module-item')
    ...
    .map((i, el) => {
      ...
      const teamUrl = process.env.VLR_URL + $(el).attr('href');
