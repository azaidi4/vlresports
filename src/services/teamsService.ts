  ...
  if (region === 'all') {
    ...
    $('tr')
      ...
      .map((i, el) => {
        ...
        const url =
          process.env.VLR_URL +
          $(el).find('td').first().next().find('a').attr('href');
        const img = $(el)
          .find('td')
          .first()
          .next()
          ...
          .includes('/img/vlr')
          ? process.env.VLR_URL +
            $(el).find('td').first().next().find('img').attr('src')
    ...
    $('.mod-scroll')
      ...
      .map((i, el) => {
        ...
        const img = $(el)
          ...
          .includes('/img/vlr')
          ? process.env.VLR_URL +
            $(el).find('a').first().find('img').attr('src')
...
export async function getTeamById(id) {
  ...
  const matchesResponse = await fetch(
    `${process.env.VLR_URL}/team/matches/${id}/?group=completed`
  );
  const matchesBody = await matchesResponse.text();
  const $matches = cheerio.load(matchesBody);
  ...
  $('.team-summary-container-2')
    ...
    .map((i, el) => {
      ...
      const event = {
        ...
        url: process.env.VLR_URL + $(el).attr('href'),
  ...
  roster.forEach((el) => {
    if ($(el).has('.wf-tag').text()) {
      ...
      const staffMember = {
        ...
        url: process.env.VLR_URL + $(el).find('a').attr('href'),
